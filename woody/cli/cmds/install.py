from woody.pywoody.core.console import WoodyError
from woody.pywoody.handlers.processor import Processor

import argparse

def install(self, args):
    """Install Woody into DCC's based on flags: -blender 'exe_path'"""
    parser = argparse.ArgumentParser(prog='install', exit_on_error=False)
    parser.add_argument('-b', '--blender', type=str, metavar='EXE')
    parser.add_argument('-f', '--force', action='store_true', help='Force update all dependencies')
    
    parsed = self._parse_args(parser, args)
    
    if parsed is None:
        return 
    
    if parsed.blender:
        Processor().install_blender(parsed.blender, parsed.force)
    
    else:
        raise WoodyError("No valid flags provided. Use -b/--blender")